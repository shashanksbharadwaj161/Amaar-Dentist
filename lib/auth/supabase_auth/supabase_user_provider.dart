...
class PatientSupabaseUser extends BaseAuthUser {
  PatientSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> patientSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PatientSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
